from utility.pixel_calculator import medium_calculator

tool_types = {
    "picaxe":0,
    "axe":1,
    "shovel":2,
    "sword":3,
    "thepowerofgod":4,
}

block_ids = {
    "dirt":0,
    "grassblock":1,
    "stone":2,
    "grimstone":3,
    "bedrock":4,
    "grass":5,
    "log":6,
    "leaves":7,
    "crafting":8,
    "planks":9,
    "red_bricks":10,
    "grey_bricks":11,
    "library":12,
    "furnace":13,
    "coal_ore":14,
    "iron_ore":15,
    "gold_ore":16,
    "diamond_ore":17,
    "coal_block":18,
    "iron_block":19,
    "gold_block":20,
    "diamond_block":21,
    "chest":22,
    "sand":23,
    "desertgrass":24,
    "glass":25,
    "littlestone":26,
    "cactus":27,
    "graveldirt":28,
    "gravel":29,
    "redleaves":30,
    "redsand":31,
    "redgrimstone":32,
    "stonegrass":33,
    "birchlog":34,
    "redwood":35,
    "stonedirt":36,
}

ores_data = {
    block_ids["coal_ore"]: {
        "range":[20,40],
        "chances":45,
    },

    block_ids["iron_ore"]: {
        "range":[36,50],
        "chances":25,
    },

    block_ids["gold_ore"]: {
        "range":[40,52],
        "chances":10,
    },

    block_ids["diamond_ore"]: {
        "range":[52,62],
        "chances":5,
    },
}

items_ids = {
    "meat":0,
    "cookedmeat":1,
    "bone":2,
    "brown_fungus":3,
    "white_fungus":4,
    "stick":5,
    "coal":6,
    "raw_iron":7,
    "raw_gold":8,
    "diamond":9,
    "iron_ingot":10,
    "gold_ingot":11
}

items_data = {
    items_ids["meat"]: {"name":"raw_meat",
                        "type":["food","smeltable"],
                        "hunger":2,
                        "result":items_ids["cookedmeat"],
                        "smelt_cooldown":3.5*1000,
                        "key":"R",
                        "hotbar_tooltip":"to eat",
                        "tooltip":"Can be eaten or cooked in the furnace. It's obtained from animals.",},

    items_ids["cookedmeat"]: {"name":"cooked_meat",
                             "type":["food"],
                             "hunger":6,
                             "key":"R",
                             "tooltip":"Can be eaten. It's obtained smelting raw meat.",},
    
    items_ids["brown_fungus"]: {"name":"brown_mushroom",
                             "type":["food"],
                             "hunger":1,
                             "key":"R",
                             "hotbar_tooltip":"to eat",
                             "tooltip":"Can be eaten. It's dropped by zombies.",},

    items_ids["white_fungus"]: {"name":"white_mushroom",
                             "type":["food"],
                             "hunger":1,
                             "key":"R",
                             "hotbar_tooltip":"to eat",
                             "tooltip":"Can be eaten. It's dropped by zombies.",},

    items_ids["bone"]: {"name":"bone",
                             "type":["misc"],
                             "key":None,
                             "tooltip":"It's dropped by skeletons.",
                             },

    items_ids["stick"]: {"name":"stick",
                             "type":["misc"],
                             "key":None,
                             "tooltip":"Useful in a lot of craftings, especially tools.",
                             },
    
    items_ids["coal"]: {"name":"coal",
                             "type":["fuel"],
                             "fuel_points":8,
                             "key":None,
                             "tooltip":"Can be used in the furnace to smelt and cook items. Can be found underground.",
                             },

    items_ids["raw_iron"]: {"name":"raw_iron",
                             "type":["ore","smeltable"],
                             "fuel":2,
                             "result":items_ids["iron_ingot"],
                             "smelt_cooldown":6*1000,
                             "key":None,
                             "tooltip":"Can be smelted in the furnace to obtain iron ingots. Can be found underground.",
                             },

    items_ids["raw_gold"]: {"name":"raw_gold",
                             "type":["ore","smeltable"],
                             "fuel":3,
                             "result":items_ids["gold_ingot"],
                             "smelt_cooldown":8*1000,
                             "key":None,
                             "tooltip":"Can be smelted in the furnace to obtain gold ingots. Can be found underground.",
                             },

    items_ids["diamond"]: {"name":"diamond",
                             "type":["misc"],
                             "key":None,
                             "tooltip":"Useful for high level craftings. Can be found deep underground.",
                             },

    items_ids["iron_ingot"]: {"name":"iron_ingot",
                             "type":["misc"],
                             "key":None,
                             "tooltip":"Useful for medium level craftings. It's obtained smelting raw iron.",
                             },

    items_ids["gold_ingot"]: {"name":"gold_ingot",
                             "type":["misc"],
                             "key":None,
                             "tooltip":"Useful for medium level craftings. It's obtained smelting raw gold.",
                             },
}

blocks_data = {
    block_ids["dirt"]:          {"name":"dirt_block",
                                "tool_required":tool_types["shovel"],
                                "mine_cooldown":0.72*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["dirt"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Can be mined with the shovel and found almost everywhere.",},

    block_ids["graveldirt"]:          {"name":"gravelly_dirt_block",
                                "tool_required":tool_types["shovel"],
                                "mine_cooldown":0.72*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["dirt"],"type":"blocks"},
                                "ignore_tool":True,},

    block_ids["grassblock"]:    {"name":"grass_block",
                                "tool_required":tool_types["shovel"],
                                "mine_cooldown":0.72*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["dirt"],"type":"blocks"},
                                "ignore_tool":True,},

    block_ids["sand"]:    {"name":"sand_block",
                                "tool_required":tool_types["shovel"],
                                "mine_cooldown":0.66*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["sand"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Can be mined with the shovel. Can be converted to glass in the crafting table.",},

    block_ids["redsand"]:    {"name":"red_sand_block",
                                "tool_required":tool_types["shovel"],
                                "mine_cooldown":0.66*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["redsand"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Can be mined with the shovel. Can be converted to glass in the crafting table.",},

    block_ids["glass"]:    {"name":"glass_block",
                                "tool_required":tool_types["thepowerofgod"],
                                "level_required":4,
                                "mine_cooldown":0.52*1000,
                                "max_cooldown":1.0,
                                "drop":{"id":block_ids["glass"],"type":"blocks"},
                                "ignore_tool":False,
                                "tooltip":"Disappear when breaking. Can be crafted with sand or red sand.",},

    block_ids["stone"]:         {"name":"stone_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.2*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["stone"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Can be mined with the picaxe. Useful for a lot of craftings. Can be found everywhere underground.",},

    block_ids["stonegrass"]:         {"name":"green_stone_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.1*1000,
                                "max_cooldown":6,
                                "drop":{"id":block_ids["stone"],"type":"blocks"},
                                "ignore_tool":True,},

    block_ids["stonedirt"]:         {"name":"brown_stone_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.1*1000,
                                "max_cooldown":6,
                                "drop":{"id":block_ids["stone"],"type":"blocks"},
                                "ignore_tool":True,},

    block_ids["gravel"]:         {"name":"gravel_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.0*1000,
                                "max_cooldown":3,
                                "drop":{"id":block_ids["gravel"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Can be mined with the picaxe. Similar to stone, can be found in mountains.",},
    
    block_ids["grimstone"]:     {"name":"grimstone_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.82*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["grimstone"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Can be mined with the picaxe. Useful for craftings, can be found deep underground in not desertic biomes.",},

    block_ids["redgrimstone"]:     {"name":"red_grimstone_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.82*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["redgrimstone"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Can be mined with the picaxe. Can be found deep underground in desertic biomes.",},

    block_ids["bedrock"]:       {"name":"bedrock",
                                "tool_required":tool_types["thepowerofgod"],
                                "mine_cooldown":999*1000,
                                "max_cooldown":999,
                                "drop":{"id":block_ids["bedrock"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"You should not have this block.",},

    block_ids["log"]:           {"name":"log_block",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":1.05*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["log"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Can be mined with the axe. Obtainable from oak trees. Can be converted in planks with the crafting table.",},

    block_ids["birchlog"]:           {"name":"birch_log_block",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":1.05*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["birchlog"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Can be mined with the axe. Obtainable from birch trees. Can be converted in red planks with the crafting table.",},

    block_ids["leaves"]:        {"name":"leaves_block",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":0.4*1000,
                                "max_cooldown":1,
                                "drop":{"id":block_ids["leaves"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Obtainable from oak trees.",},

    block_ids["cactus"]:        {"name":"cactus_block",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":0.6*1000,
                                "max_cooldown":1.1,
                                "drop":{"id":block_ids["cactus"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Obtainable from cactuses in deserts.",},

    block_ids["redleaves"]:        {"name":"birch_leaves_block",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":0.4*1000,
                                "max_cooldown":1,
                                "drop":{"id":block_ids["redleaves"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Obtainable from birch trees.",},

    block_ids["crafting"]:        {"name":"crafting_table",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":1.0*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["crafting"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Used for crafting. Place and right click to use.",},

    block_ids["chest"]:        {"name":"chest",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":1.1*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["chest"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Used to store items. Place and right click to use.",},

    block_ids["planks"]:        {"name":"oak_planks",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":0.85*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["planks"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Useful for craftings.",},

    block_ids["redwood"]:        {"name":"birch_planks",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":0.85*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["redwood"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Convert to oak wood to use in craftings.",},

    block_ids["red_bricks"]:        {"name":"red_bricks",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.2*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["red_bricks"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Building block.",},

    block_ids["grey_bricks"]:        {"name":"dark_bricks",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.5*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["grey_bricks"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Building block.",},

    block_ids["library"]:        {"name":"bookshelf",
                                "tool_required":tool_types["axe"],
                                "mine_cooldown":1.0*1000,
                                "max_cooldown":1.5,
                                "drop":{"id":block_ids["library"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Contains books.",},

    block_ids["furnace"]:        {"name":"furnace",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.3*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["furnace"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"Used to smelt items. Place and right click to use.",},

    block_ids["coal_ore"]:        {"name":"coal_ore",
                                "tool_required":tool_types["picaxe"],
                                "level_required":0,
                                "mine_cooldown":1.2*1000,
                                "max_cooldown":8,
                                "drop":{"id":items_ids["coal"],"type":"items"},
                                "ignore_tool":False,},

    block_ids["iron_ore"]:        {"name":"iron_ore",
                                "tool_required":tool_types["picaxe"],
                                "level_required":1,
                                "mine_cooldown":1.3*1000,
                                "max_cooldown":8,
                                "drop":{"id":items_ids["raw_iron"],"type":"items"},
                                "ignore_tool":False,},

    block_ids["gold_ore"]:        {"name":"gold_ore",
                                "tool_required":tool_types["picaxe"],
                                "level_required":2,
                                "mine_cooldown":1.4*1000,
                                "max_cooldown":8,
                                "drop":{"id":items_ids["raw_gold"],"type":"items"},
                                "ignore_tool":False,},

    block_ids["diamond_ore"]:        {"name":"diamond_ore",
                                "tool_required":tool_types["picaxe"],
                                "level_required":2,
                                "mine_cooldown":1.5*1000,
                                "max_cooldown":8,
                                "drop":{"id":items_ids["diamond"],"type":"items"},
                                "ignore_tool":False,},

    block_ids["coal_block"]:        {"name":"coal_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.2*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["coal_block"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"A lot of coal. Useful to save space in storages.",},

    block_ids["iron_block"]:        {"name":"iron_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.3*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["iron_block"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"A lot of iron. Useful to save space in storages.",},

    block_ids["gold_block"]:        {"name":"gold_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.4*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["gold_block"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"A lot of gold. Useful to save space in storages.",},

    block_ids["diamond_block"]:        {"name":"diamond_block",
                                "tool_required":tool_types["picaxe"],
                                "mine_cooldown":1.5*1000,
                                "max_cooldown":8,
                                "drop":{"id":block_ids["diamond_block"],"type":"blocks"},
                                "ignore_tool":True,
                                "tooltip":"A lot of diamond. Useful to save space in storages.",},}

frames = {
    block_ids["grass"]: 4,
    block_ids["furnace"]:2,
    block_ids["coal_ore"]:2,
    block_ids["iron_ore"]:2,
    block_ids["gold_ore"]:2,
    block_ids["diamond_ore"]:2,
    block_ids["desertgrass"]:2,
    block_ids["littlestone"]:2,
}

tools_data = {
    tool_types["picaxe"]:   {0:{"name":"wood_picaxe","damage":1,"durability":50,"attack_cooldown":0.5*1000,"message":"coal"},
                            1:{"name":"stone_picaxe","damage":2,"durability":160,"attack_cooldown":0.5*1000,"message":"iron"},
                            2:{"name":"iron_picaxe","damage":2,"durability":430,"attack_cooldown":0.6*1000,"message":"gold and diamond"},
                            3:{"name":"gold_picaxe","damage":3,"durability":710,"attack_cooldown":0.6*1000,"message":None},
                            4:{"name":"diamond_picaxe","damage":4,"durability":1520,"attack_cooldown":0.7*1000,"message":None},
                            "tooltip":"Can mine stone based blocks faster.",
                            "start_tooltip":" Required to obtain ",},
    
    tool_types["axe"]:   {0:{"name":"wood_axe","damage":2,"durability":50,"attack_cooldown":1*1000},
                            1:{"name":"stone_axe","damage":3,"durability":160,"attack_cooldown":1.1*1000},
                            2:{"name":"iron_axe","damage":4,"durability":430,"attack_cooldown":1.2*1000},
                            3:{"name":"gold_axe","damage":5,"durability":710,"attack_cooldown":1.2*1000},
                            4:{"name":"diamond_axe","damage":6,"durability":1520,"attack_cooldown":1.3*1000},
                            "tooltip":"Can mine wood based blocks faster.",
                            "start_tooltip":None,},
    
    tool_types["shovel"]:   {0:{"name":"wood_shovel","damage":1,"durability":50,"attack_cooldown":0.4*1000},
                            1:{"name":"stone_shovel","damage":1,"durability":160,"attack_cooldown":0.4*1000},
                            2:{"name":"iron_shovel","damage":2,"durability":430,"attack_cooldown":0.5*1000},
                            3:{"name":"gold_shovel","damage":2,"durability":710,"attack_cooldown":0.5*1000},
                            4:{"name":"diamond_shovel","damage":3,"durability":1520,"attack_cooldown":0.5*1000},
                            "tooltip":"Can mine dirt based blocks faster.",
                            "start_tooltip":None,},
    
    tool_types["sword"]:   {0:{"name":"wood_sword","damage":3,"durability":50,"attack_cooldown":0.6*1000},
                            1:{"name":"stone_sword","damage":4,"durability":160,"attack_cooldown":0.6*1000},
                            2:{"name":"iron_sword","damage":5,"durability":430,"attack_cooldown":0.7*1000},
                            3:{"name":"gold_sword","damage":6,"durability":710,"attack_cooldown":0.7*1000},
                            4:{"name":"diamond_sword","damage":8,"durability":1520,"attack_cooldown":0.8*1000},
                            "tooltip":"Can do more damage to entities.",
                            "start_tooltip":None,},
}

entities_data = {
    "porcupine":{"name":"porcupine",
                "type":"animal",
                "speed":medium_calculator(3),
                "health":10,
                "chances":5},

    "zombie":{"name":"zombie",
                "type":"monster",
                "speed":medium_calculator(3),
                "health":20,
                "chances":5,
                "target_range":20,
                "attack_range":1,
                "attack_damage":1,
                "attack_cooldown":2*1000},

    "skeleton":{"name":"skeleton",
                "type":"monster",
                "speed":medium_calculator(4),
                "health":20,
                "chances":3,
                "target_range":30,
                "attack_range":2,
                "attack_damage":2,
                "attack_cooldown":4*1000},
}

biomes_ids = {
    "plains":0,
    "forest":1,
    "desert":2,
    "birchforest":3,
    "reddesert":4,
    "mountain":5,
    "birchmountain":6,
}

trees_ids = {
    "oak_tree":0,
    "birch_tree":1,
    "cactus":2,
}

biomes_data = {
    biomes_ids["plains"] : {
        "name":"plains",
        "noise_data":{"height_multiplier":4,"amplitude_multiplier":0.05},
        "top_layer":block_ids["grassblock"],
        "bottom_layer":[block_ids["dirt"]],
        "deep_layer":block_ids["grimstone"],
        "tree_type":trees_ids["oak_tree"],
        "tree_chances":7,
        "grass_chances":35,
        "grass_type":block_ids["grass"],
        "animal_entities":["porcupine"],
    },
    biomes_ids["forest"] : {
        "name":"forest",
        "noise_data":{"height_multiplier":6,"amplitude_multiplier":0.07},
        "top_layer":block_ids["grassblock"],
        "bottom_layer":[block_ids["dirt"],block_ids["graveldirt"]],
        "deep_layer":block_ids["grimstone"],
        "tree_type":trees_ids["oak_tree"],
        "tree_chances":50,
        "grass_chances":10,
        "grass_type":block_ids["grass"],
        "animal_entities":["porcupine"],
    },
    biomes_ids["birchforest"] : {
        "name":"birch_forest",
        "noise_data":{"height_multiplier":6,"amplitude_multiplier":0.07},
        "top_layer":block_ids["grassblock"],
        "bottom_layer":[block_ids["dirt"],block_ids["graveldirt"]],
        "deep_layer":block_ids["grimstone"],
        "tree_type":trees_ids["birch_tree"],
        "tree_chances":50,
        "grass_chances":10,
        "grass_type":block_ids["grass"],
        "animal_entities":["porcupine"],
    },
    biomes_ids["mountain"] : {
        "name":"mountain",
        "noise_data":{"height_multiplier":12,"amplitude_multiplier":0.085},
        "top_layer":block_ids["stonegrass"],
        "bottom_layer":[block_ids["stone"],block_ids["gravel"]],
        "deep_layer":block_ids["grimstone"],
        "tree_type":trees_ids["oak_tree"],
        "tree_chances":30,
        "grass_chances":15,
        "grass_type":block_ids["littlestone"],
        "animal_entities":["porcupine"],},

    biomes_ids["birchmountain"] : {
        "name":"birch_mountain",
        "noise_data":{"height_multiplier":11,"amplitude_multiplier":0.085},
        "top_layer":block_ids["stonedirt"],
        "bottom_layer":[block_ids["stone"],block_ids["gravel"]],
        "deep_layer":block_ids["grimstone"],
        "tree_type":trees_ids["birch_tree"],
        "tree_chances":30,
        "grass_chances":15,
        "grass_type":block_ids["littlestone"],
        "animal_entities":["porcupine"],
    },
    biomes_ids["desert"] : {
        "name":"desert",
        "noise_data":{"height_multiplier":4,"amplitude_multiplier":0.03},
        "top_layer":block_ids["sand"],
        "bottom_layer":[block_ids["sand"]],
        "deep_layer":block_ids["redgrimstone"],
        "tree_type":trees_ids["cactus"],
        "tree_chances":20,
        "grass_chances":20,
        "grass_type":block_ids["desertgrass"],
        "animal_entities":["porcupine"],
    },

    biomes_ids["reddesert"] : {
        "name":"red_desert",
        "noise_data":{"height_multiplier":4,"amplitude_multiplier":0.03},
        "top_layer":block_ids["redsand"],
        "bottom_layer":[block_ids["redsand"]],
        "deep_layer":block_ids["redgrimstone"],
        "tree_type":trees_ids["cactus"],
        "tree_chances":15,
        "grass_chances":20,
        "grass_type":block_ids["desertgrass"],
        "animal_entities":["porcupine"],
    },
}

